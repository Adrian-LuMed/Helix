...
if (!condoId) { console.log('Usage: node tests/_cleanup.js <condoId>'); process.exit(1); }
...
ws.on('message', (raw) => {
  ...
  if (msg.type === 'res' && !authDone) {
    ...
    ws.send(JSON.stringify({ type: 'req', id: 'del', method: 'condos.delete', params: { id: condoId } }));
    return;
  }
  if (msg.id === 'del') { console.log('Deleted condo ' + condoId + ':', msg.ok ? 'OK' : JSON.stringify(msg.error)); ws.close(); }
});
