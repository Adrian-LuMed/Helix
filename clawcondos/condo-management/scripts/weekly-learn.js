#!/usr/bin/env node
// clawcondos/condo-management/scripts/weekly-learn.js
//
...
if (suggestions.length === 0) {
  console.log('No learning suggestions (need 2+ corrections to same condo).');
  process.exit(0);
}

console.log(`Found ${suggestions.length} suggestion(s):`);
for (const s of suggestions) {
  console.log(`  ${s.condoId}: ${s.correctionCount} corrections`);
...
if (applied.length === 0) {
  console.log('No changes to apply (condos may not exist or keywords already present).');
} else {
  console.log(`${apply ? 'Applied' : 'Would apply'} ${applied.length} change(s):`);
  for (const a of applied) {
    console.log(`  ${a.condoName} (${a.condoId}): +${a.addedKeywords.join(', ')}`);
  }
}
